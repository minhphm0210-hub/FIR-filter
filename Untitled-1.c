...
// hàm matched filter
void matched_filter(double input[], double output[], int length) {
    for (int i = 0; i < length; i++) {
        output[i] = 0;

        for (int m = 0; m < 8; m++) {
            if (i - m >= 0) {
                output[i] += input [i - m]; // phép cộng dồn
            }
        }
    }
}
...
int main()
{
    ...
    double after_matchedfilter[N * L];
    ...
    // xuất tín hiệu sau khi được lọc bởi FIR filter để đưa vào Matlab
    FILE *f1 = fopen("D:\\FIR\\after_FIR.txt", "w");
    ...
    // gọi hàm match_filter, với input là tín hiệu sau khi được lọc bởi FIR, output được xếp vào mảng after_Matchedfilter
    matched_filter(after_FIR, after_matchedfilter, N * L);


    // xuất tín hiệu sau khi được lọc bởi Matched filter
    FILE *f2 = fopen("D:\\FIR\\after_matched_filter.txt", "w");

    for (int i = 0; i < N * L; i++)
    {
        fprintf(f1, "%f\n", after_matchedfilter[i]);
    }
